...
function stErrInfo(err: unknown): NormalizedStorageErr | undefined {
  ...
  // Узкое приведение через структурные проверки (без any)
...
export async function POST(req: Request) {
  try {
    ...
    if (missing.length > 0) {
      ...
      return jsonError('Server misconfigured: missing env vars', 500, { missingEnv: missing });
    }
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      const uploadErrors: Array<{ file: string; error: ReturnType<typeof stErrInfo> | { message: string } }> = [];
      ...
      for (const file of files) {
        try {
          ...
          const key = `tickets/${ticket.id}/${Date.now()}_${randomUUID()}.${ext}`;

          const up = await supabaseServer.storage.from(BUCKET).upload(key, buffer, {
          ...
          if (up.error) {
            ...
            uploadErrors.push({ file: file.name, error: info ?? { message: 'Unknown storage error' } });
      ...
      if (uploadedPaths.length > 0) {
        const upd = await supabaseServer.from('tickets').update({ attachments: uploadedPaths }).eq('id', ticket.id);
        if (upd.error) logError('Attachment update error:', sbErrInfo(upd.error));
      }
    ...
    const ins = await supabaseServer.from('tickets').insert(parsed.data).select('id').single();
    if (ins.error || !ins.data) {
      const info = sbErrInfo(ins.error);
    ...
    return jsonError('Unexpected error', 500, DEBUG ? { message: msg } : undefined);
  }
}
