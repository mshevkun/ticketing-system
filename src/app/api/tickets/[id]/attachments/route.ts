...
// Add attachments to an existing ticket (requester or IT staff)
export async function POST(
  ...
  try {
    ...
    if (!isIT && !isCreator) {
      return NextResponse.json(
        { error: "Only the ticket creator or IT staff can add attachments" },
        { status: 403 }
      );
    }
    ...
    for (const file of validFiles) {
      ...
      const key = `tickets/${ticketId}/${Date.now()}_${randomUUID()}.${ext}`;
...
// Remove one attachment from a ticket (requester or IT staff)
export async function DELETE(
  ...
  try {
    ...
    if (!isIT && !isCreator) {
      return NextResponse.json(
        { error: "Only the ticket creator or IT staff can remove attachments" },
        { status: 403 }
      );
    }
