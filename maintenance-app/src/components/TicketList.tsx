...
export default function TicketList({
  ...
  const fetchTickets = useCallback(async () => {
    ...
    let query = supabase.from("maintenance_tickets").select("*").order("created_at", {
      ascending: false,
    });

    if (!MAINTENANCE_EMAILS.includes(userEmail)) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-4 sm:mb-6 px-4 sm:px-0">
        <div>
          <h2 className="text-xl sm:text-2xl font-semibold">
            {MAINTENANCE_EMAILS.includes(userEmail || "") ? "All Requests" : "My Requests"}
          </h2>
          <p className="text-xs sm:text-sm text-gray-600 mt-0.5">Track and manage your maintenance requests</p>
        </div>
        {tickets.length > 1 && (
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-8 sm:p-12 text-center shadow-sm mx-4 sm:mx-0">
          ...
          <p className="text-xs sm:text-sm text-gray-600">
            {MAINTENANCE_EMAILS.includes(userEmail || "")
              ...
              : "You don't have any maintenance requests yet. Create your first request above."}
          </p>
        </div>
      ) : (
        <>
          ...
          {sortedTickets.map((t) => (
            <li key={t.id}>
              <Link
                href={`/maintenance-system/tickets/${t.id}`}
