...
function TicketingSystemPageInner() {
  ...
  useEffect(() => {
    ...
    const redirectTo = `${window.location.origin}/maintenance-system`;
  ...
  const loginWithMicrosoft = async () => {
    if (isInIframe) {
      const url = `${window.location.origin}/maintenance-system?${AUTH_REDIRECT_PARAM}=${AUTH_REDIRECT_VALUE}`;
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-base sm:text-xl md:text-2xl font-bold text-gray-900 flex items-center gap-1.5 sm:gap-2">
              ...
              <span className="hidden sm:inline">
                People USA Maintenance Ticketing System
              </span>
              <span className="sm:hidden">Maintenance System</span>
            </h1>
            ...
            <div className="flex flex-row items-center gap-2 sm:gap-4 w-full sm:w-auto justify-between sm:justify-end">
              <div className="text-left sm:text-right min-w-0 flex-1 sm:flex-none">
                ...
                <p className="text-[10px] sm:text-xs text-gray-500">
                  {MAINTENANCE_EMAILS.includes(userEmail) ? "Maintenance Staff" : "Employee"}
                </p>
              </div>
              <button
                onClick={logout}
          ...
          <div className="max-w-xl mx-auto">
            <div className="bg-white border border-gray-200 rounded-xl p-6 sm:p-8 shadow-sm text-center">
              <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-2">
                Welcome to Maintenance Ticketing System
              </h2>
              <p className="text-sm sm:text-base text-gray-600 mb-6">
                {isInIframe
                  ? "Open the app in your browser to sign in with Microsoft 365 and use the ticketing system."
                  : "Sign in with your Microsoft 365 account to create and manage maintenance requests"}
              </p>
              <button
