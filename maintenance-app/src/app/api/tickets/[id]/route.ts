...
export async function PATCH(
  ...
  try {
    ...
    // Only the ticket creator can edit the description (not maintenance staff)
...
// Delete a ticket (only creator or maintenance staff can delete)
export async function DELETE(
  ...
  try {
    ...
    // Check authorization: maintenance staff OR ticket creator
    ...
    if (!isITStaff && !isCreator) {
      return NextResponse.json(
        { error: "Forbidden: You can only delete your own tickets or must be maintenance staff" },
        { status: 403 }
      );
    }
